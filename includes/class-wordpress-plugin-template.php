...
class WordPress_Plugin_Template {
	...
	/**
	 * DressCode Skills manager.
	 *
	 * @var DressCode_Skills|null
	 * @access public
	 * @since  1.0.0
	 */
	public $skills = null;
	...
	/**
	 * DressCode Editor integration.
	 *
	 * @var DressCode_Editor|null
	 * @access public
	 * @since  1.0.0
	 */
	public $editor = null;
	...
	public function admin_enqueue_styles( $hook = '' ) {
		$css_path = trailingslashit( $this->assets_dir ) . 'css/admin.css';
		$ver      = file_exists( $css_path ) ? filemtime( $css_path ) : $this->_version;
		wp_register_style( $this->_token . '-admin', esc_url( $this->assets_url ) . 'css/admin.css', array(), $ver );
	...
	public function admin_enqueue_scripts( $hook = '' ) {
		$js_path = trailingslashit( $this->assets_dir ) . 'js/admin' . $this->script_suffix . '.js';
		$ver     = file_exists( $js_path ) ? filemtime( $js_path ) : $this->_version;
		wp_register_script( $this->_token . '-admin', esc_url( $this->assets_url ) . 'js/admin' . $this->script_suffix . '.js', array( 'jquery' ), $ver, true );
	...
	public function load_localisation() {
		load_plugin_textdomain( 'dresscode', false, dirname( plugin_basename( $this->file ) ) . '/lang/' );
	...
	public function install() {
		...
		// Create the skills table, migrate legacy rows to folders, seed default.
		if ( class_exists( 'DressCode_Skills' ) ) {
			$skills = new DressCode_Skills();
			$skills->upgrade();
			update_option( 'dresscode_db_version', DressCode_Skills::DB_VERSION );
		}
	} // End install ()
